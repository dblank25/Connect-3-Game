...
class Connect3Board:
    ...
    def winner(self): # returns the winning label
        for i in range(COLS):
            for j in range(ROWS):
                label = self.get(i, j)
                if label != EMPTY:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if cmd == 'print':
            ...
            if (sum([i.count("X") for i in board.b])) > (sum([i.count("O") for i in board.b])):
                print(stringify_boards(board.next("O")))
            else:
                print(stringify_boards(board.next("X")))
        elif cmd == 'random':
            game = Game(board)
            print(stringify_boards(game.randomGame()))
        elif cmd == 'minimax':
            game = Game(board)
            print(stringify_boards(game.minimaxGame()))
        elif cmd == 'alphabeta':
            game = Game(board)
            print(stringify_boards(game.alphabetaGame()))
        else:
