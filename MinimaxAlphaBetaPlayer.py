...
class MinimaxAlphaBetaPlayer:
    ...
    def move(self, state):
        ...
        for board in state.next(self.label):
            utility = self.minValue(board, depth, -math.inf, math.inf)
            if utility > maxval:
                maxval = utility
                myboard = board
        end = time.time()
        print((end-start), " seconds per alpha-beta player move")
        return myboard
    ...
    def maxValue(self,state,depth, a, b): # returns utility value
        if state.winner():
            return self.utility(state, state.winner(),depth)
        v = -math.inf
        for s in state.next('O'):
            v = max(v, self.minValue(s, depth+1, a, b))
            if v >= b:
                return v
            a = max(a, v)
        return v
    ...
    def minValue(self, state, depth, a, b):
        ...
        for s in state.next('X'):
            v = min(v, self.maxValue(s, depth+1, a, b))
            if v <= a:
                return v
            b = min(b, v)
        return v
